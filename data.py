        ...
        for (noun_phrase, sentiment) in zip(temp_df["NP"], temp_df["sentiment"]):
            ...
            if len(noun_phrase.split(" ")) < 2:
                try:
                    ...
                    indices = [(m.start(0)+1, m.end(0)) for m in re.finditer(" " + noun_phrase, main_post)]
                    indices.extend([(m.start(0), m.end(0)-1) for m in re.finditer(noun_phrase + " ", main_post)])
                except:
                    indices = [(m.start(0), m.end(0)) for m in re.finditer(noun_phrase, main_post)]
            else:
                indices = [(m.start(0), m.end(0)) for m in re.finditer(noun_phrase, main_post)]
            ...
            all_data[main_post].append((indices, real_label))
    return all_data
